globals [ new-products products badx bady stop-flag bad-counter good-counter]
...
to go
  ...
  [
    ...
    [
      show "Trideset proizvoda uspješno proizvedeno!"       ;; ukoliko je manje od 10% neispravnih serija je uspjela
      stop
    ]
    tick
  ]
...
to walk                  ;; funkcija pomicanja proizvoda po traci
  ...
  [
    if xcor = 6 and not (color = red)
    [
      set good-counter (good-counter + 1)
      show (word "Broj uspješnih proizvoda: " good-counter)
      die
    ]
    if xcor = 6 and (color = red)[
      set bad-counter (bad-counter + 1)
      show (word "Broj loših proizvoda: " bad-counter)
      die
    ]
    fd 1
  ]
...
to check-history
  ...
  [
    ...
    show "Tri neuspješna proizvoda pronađena!!! Obustavlja se proizvodnja."
