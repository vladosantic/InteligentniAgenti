...
to kill-fish
  if any? turtles with [breed = pray][
    ask predator[
      set shark-patch one-of patches in-radius 3 with [any? turtles-here with [breed = pray]]
      if shark-patch != nobody[
        move-to shark-patch
        ask pray-here[
          die
        ]
        move-to min-one-of patches with [abs pxcor = 8 or abs pycor = 6][distance myself]
      ]
    ]
  ]
end
