globals [ shark-patch shark-patch2 movement]
...
to go
  if not any? turtles with [breed = pray][
    stop
  ]
  predator-move
  predator-move2
...
to predator-move
  if any? turtles with [breed = pray][
    ask turtle 0[
      set shark-patch min-one-of patches with [any? turtles-here with [breed = pray]][distance myself]
      if shark-patch != nobody[
        face shark-patch
        fd 1
        if any? pray-here [
          ask pray-here[
...
to predator-move2
  if any? turtles with [breed = pray][
    ask turtle 2[
      set shark-patch2 min-one-of patches with [any? turtles-here with [breed = pray]][distance myself]
      if shark-patch2 != nobody[
        face shark-patch2
        fd 1
        if any? pray-here [
          ask pray-here[
...
to baby-predator
  ask turtle 1[
    while [patch-here != shark-patch][
      face shark-patch
      fd 1
    ]
    if patch-here = shark-patch[
...
to baby-predator2
  ask turtle 3[
    while [patch-here != shark-patch2][
      face shark-patch2
      fd 1
    ]
    if patch-here = shark-patch2[
      while[patch-here != patch 8 -6][
        face patch 8 -6
        fd 1
      ]
...
Napraviti modifikaciju prethodnog primjera za situaciju kada je se u sustavu nalaze dva para agenta vrste predator (roditelj i dijete) i dva agenta vrste plijen. Svaki predator roditelj, nakon lova, poziva svoje dijete.
@#$#@#$#@
default
true
0
