globals [ samples1 colorlist target-patch samples-coords templist sample1 x colors y]
...
to agent1 ;; funkcija agenta 1 koji trazi uzorke i poziva agenta 2 kada pronadje uzorak
  if x = 3 [
    ask turtle 0 [ move-to patch -8 -6 ]
    ask turtle 1 [ agent2 ]
  ]
  set target-patch min-one-of patches with [member? pcolor samples1] [distance myself]
  ask turtle 0 [
  if target-patch != nobody  [
    ...
    if ([pcolor] of patch-here = orange)
      [
        if (member? orange samples1 ) [
          set templist lput pxcor templist
          set templist lput pycor templist
          set samples-coords lput templist samples-coords
          set samples1 remove orange samples1
          set x x + 1
        ]
      ]
    if ([pcolor] of patch-here = red)
      [
        if (member? red samples1 ) [
          set templist lput pxcor templist
          set templist lput pycor templist
          set samples-coords lput templist samples-coords
          set samples1 remove red samples1
          set x x + 1
        ]
      ]
  if ([pcolor] of patch-here = blue)
      [
        if (member? blue samples1 ) [
          set templist lput pxcor templist
          set templist lput pycor templist
          set samples-coords lput templist samples-coords
          set samples1 remove blue samples1
          set x x + 1
        ]
      ]
  ]
...
to agent2 ;; funkcija kretanja agenta 2
  if not empty? samples-coords[
    set sample1 last samples-coords
    let heading-patch patch item 0 sample1 item 1 sample1
    face heading-patch
    fd 1

    if ([pcolor] of patch-here = orange) [
      if not (member? "Orange" colors ) [
        set colors lput "Orange" colors
        set y y + 1
      ]
    ]
    if ([pcolor] of patch-here = red) [
      if not (member? "Red" colors ) [
        set colors lput "Red" colors
        set y y + 1
      ]
    ]
    if ([pcolor] of patch-here = blue) [
      if not (member? "Blue" colors ) [
        set colors lput "Blue" colors
        set y y + 1
      ]
    ]

    let helplist[]
    set helplist lput pxcor helplist
    set helplist lput pycor helplist
