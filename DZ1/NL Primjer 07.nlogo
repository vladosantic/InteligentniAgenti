...
to go
  if (all? patches [pcolor = black]) [show "Done" stop]
  tick
  ask turtles
  [
    if not empty? model
...
to clean
    let target-patch min-one-of (patches in-radius 20 with [pcolor = grey]) [distance myself]
    ...
    set model remove helpList model
    show model
  ]
end
@#$#@#$#@
